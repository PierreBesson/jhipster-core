...
const { deployments: DeploymentErrors } = require('../exceptions/error_cases');
...
class JDLDeployment {
  ...
  static checkValidity(deployment) {
    const errors = [];
    if (!deployment) {
      errors.push(DeploymentErrors.NoDeployment);
      return errors;
    }
    if (!deployment.deploymentType) {
      errors.push(DeploymentErrors.NoDeploymentType);
    }
    if (!deployment.appsFolders) {
      errors.push(DeploymentErrors.NoApplications);
    }
    if (!deployment.dockerRepositoryName) {
      errors.push(DeploymentErrors.NoRepository);
    }
    return errors;
  }
