...
const { applications: ApplicationErrors } = require('../exceptions/error_cases');
...
class AbstractJDLApplication {
  ...
  static checkValidity(application) {
    ...
    if (!application || !application.config) {
      errors.push(ApplicationErrors.NoApplication);
      return errors;
    }
    if (!application.config.baseName) {
      errors.push(ApplicationErrors.NoName);
    }
    if (!application.config.authenticationType) {
      errors.push(ApplicationErrors.NoAuthenticationType);
    }
    if (!application.config.buildTool) {
      errors.push(ApplicationErrors.NoBuildTool);
    }
    if (application.config.enableTranslation && !application.config.nativeLanguage) {
      errors.push(ApplicationErrors.NoChosenLanguage);
    }
    return errors;
  }
