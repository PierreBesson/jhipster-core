...
const { relationships: RelationshipErrors } = require('../exceptions/error_cases');
...
class JDLRelationship {
  ...
  static checkValidity(relationship) {
    const errors = [];
    if (!relationship) {
      errors.push(RelationshipErrors.NoRelationship);
      return errors;
    }
    if (!RelationshipTypes.exists(relationship.type)) {
      errors.push(`${RelationshipErrors.WrongType}: got '${relationship.type}'`);
    }
    return errors;
  }
