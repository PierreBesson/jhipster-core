...
const JDLEntityValidator = require('./validators/jdl_entity_validator');
...
class JDLObject {
  ...
  addApplication(application) {
    if (!application) {
      throw new Error('An application has to be passed to be added to the JDL object.');
    }
  ...
  addEntity(entity) {
    if (!entity) {
      throw new Error('An entity has to be passed to be added to the JDL object.');
    }
    const errors = JDLEntityValidator.validate(entity);
    if (errors.length !== 0) {
