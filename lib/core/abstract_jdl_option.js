...
const { options: OptionErrors } = require('../exceptions/error_cases');
...
class AbstractJDLOption {
  ...
  static checkValidity(object) {
    const errors = [];
    if (!object) {
      errors.push(OptionErrors.NoOption);
      return errors;
    }
    if (!object.name) {
      errors.push(OptionErrors.NoName);
    }
    if (!object.entityNames) {
      errors.push(OptionErrors.NoEntityNames);
    }
    if (object.entityNames && object.entityNames.has(null)) {
      errors.push(OptionErrors.NilInEntityNames);
    }
    if (!object.excludedNames) {
      errors.push(OptionErrors.NoExcludedNames);
    }
    if (object.excludedNames && object.excludedNames.has(null)) {
      errors.push(OptionErrors.NilInExcludedNames);
    }
    try {
      object.getType();
    } catch (error) {
      errors.push(OptionErrors.NoType);
    }
    return errors;
  }
